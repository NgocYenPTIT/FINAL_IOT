...
//   // Parse the message as JSON
  ...
  if (doc.containsKey("action_from_nodeJS"))
  {
  ...
  else if(doc.containsKey("action_from_nodeJSS")){
         digitalWrite(fan, LOW);
         digitalWrite(ac, LOW);
         digitalWrite(light, LOW);
        // StaticJsonDocument<256> doc;
   ...
   else if(doc.containsKey("action_from_nodeJSSS")){
         digitalWrite(fan, HIGH);
         digitalWrite(ac, HIGH);
         digitalWrite(light, HIGH);
  }
 ...
 void loop()
 {
   ...
   unsigned long currentMillis = millis();
    if(currentMillis - lastMsgTime2 >= 500 )
   {

       if(isNhay){
        digitalWrite(add , digitalRead(add)  == HIGH ? LOW  : HIGH) ;
...
// mosquitto_pub -h 172.20.10.2 -p 2222 -u b21dccn809 -P 123456 -t "iot_NgocYen" -m "{\"action_from_nodeJS\": true, \"device\": \"fan\", \"action\": \"on\"}"

// mosquitto_pub -h 172.20.10.2 -p 2222 -u b21dccn809 -P 123456 -t "iot_NgocYen" -m "{\"action_from_nodeJSS\": true}"
// mosquitto_pub -h 172.20.10.2 -p 2222 -u b21dccn809 -P 123456 -t "iot_NgocYen" -m "{\"action_from_nodeJSSS\": true}"