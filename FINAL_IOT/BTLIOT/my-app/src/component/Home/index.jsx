...
export default function Home() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const cntDust = await fetchGet('http://localhost:8888/mqtt/get-cnt-dust');
        dispatch(IsActiveActions.changeCntDust(cntDust));
        if (second % 2 === 1) {
  ...
  React.useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      const newDust = state?.dataChart?.dust?.[11];

      if (!state?.isAlert && newDust > 80 && state.page === 1) {
        openNotification();
        dispatch(IsActiveActions.changeStatusAC(!state.statusAC))
        dispatch(IsActiveActions.changeStatusLight(!state.statusLight))
        dispatch(IsActiveActions.changeStatusFan(!state.statusFan))

        setTimeout(() => dispatch(IsActiveActions.changeStatusAC(!state.statusAC)), 1000)
        setTimeout(() => dispatch(IsActiveActions.changeStatusLight(!state.statusLight), 1200))
        setTimeout(() => dispatch(IsActiveActions.changeStatusFan(!state.statusFan)), 1400)

        setTimeout(() => dispatch(IsActiveActions.changeStatusAC(!state.statusAC)), 1600)
        setTimeout(() => dispatch(IsActiveActions.changeStatusLight(!state.statusLight), 2000))
        setTimeout(() => dispatch(IsActiveActions.changeStatusFan(!state.statusFan)), 2200)

        // setTimeout(() => dispatch(IsActiveActions.changeStatusAC(!state.statusAC)), 3000)
        // setTimeout(() => dispatch(IsActiveActions.changeStatusLight(!state.statusLight), 3000))
        // setTimeout(() => dispatch(IsActiveActions.changeStatusFan(!state.statusFan)), 3000)

        // setTimeout(() => dispatch(IsActiveActions.changeStatusAC(!state.statusAC)), 2000)
        // setTimeout(() => dispatch(IsActiveActions.changeStatusLight(!state.statusLight), 2000))
        // setTimeout(() => dispatch(IsActiveActions.changeStatusFan(!state.statusFan)), 2000)
      }
  ...
  const openNotification = () => {
    api.error({
      ...
      onClose: () => {
        ...
        setTimeout(() => dispatch(IsActiveActions.changeAlert(false)), 1000);
      }
    });
    dispatch(IsActiveActions.changeAlert(true))
  };
  return (
