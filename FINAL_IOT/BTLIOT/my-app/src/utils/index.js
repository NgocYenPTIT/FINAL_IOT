...
export const SerializerDBSensor = (data, currentPage, pageSize) => {
    const convert = (data) => {
        ...
        for (let i = 0; i < data.length; i++) {
            const x = data[i];
            ans.push({ STT: (currentPage - 1) * pageSize + i + 1, ID: x.id, Temp: x.temperature, Humid: x.humid, Light: x.light, Dust: x.dust, Time: convertISOToCustomFormatWithTimezone(x.createdAt) });
        }
        return ans;
    }
